...
\item
  ...
  as useful as plaintext, and if you can only reproduce the problem in an
  IDE but not from the command line, then the problem is with the IDE\@.
\item
...
In the bash shell, putting the following command in your \code{.bashrc} file
...
instructions.
% To build the manual, you will need \LaTeX and
% {\hevea} (\myurl{http://hevea.inria.fr/}) installed, and either
% \<rsvg-convert> or \<convert>.
...
\ahref{http://homes.cs.washington.edu/~mernst/advice/github-pull-request.html}{How
...
Instructions for writing new test cases appears in file
...
``EnerJ: Approximate Data Types for Safe and General Low-Power Computation''~\cite{SampsonDFGCG2011} (PLDI 2011, \myurl{http://www.cs.cornell.edu/~asampson/media/papers/enerj-pldi2011.pdf})
